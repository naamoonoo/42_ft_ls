...
int	main(int ac, char *av[])
{
	...
	// 	{
	// 		printf("%s:\n", ls.fi->name);
	// 		while((dp = readdir(ls.fi->dirp)) != NULL)
	// 			ft_start_with(dp->d_name, '.') ? 0 : printf("%s\t", dp->d_name);
	// 		printf("\n\n");
	// 	}
	// 	else
	// 		printf("%s\t%s\n", ft_itoa_base(ls.fi->info.st_mode,2,0), ls.fi->name);
	// 	ls.fi = ls.fi->next;
	// }
	// if (av[1])
	// 	flag_handelr(av[1]);
